...
router.post("/:id/preview-video-token", requireRole("ADMIN"), ProductController.getLessonPreviewToken);

// Detalle, actualización y borrado por id (rutas con :id al final)
router.get("/:id/preview", ProductController.getProductPreview);
router.get("/:id/analytics", requireRole("CREATOR"), ProductController.getProductAnalytics);
